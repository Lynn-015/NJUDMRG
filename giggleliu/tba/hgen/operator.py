...
class Operator(object):
    ...
    def __str__(self):
        ...
        for i in xrange(len(self.suboperators)):
            ...
            if i!=0:# and (real(operator.weight)>=0 or imag(operator.weight)!=0):
                txt+=' + '
...
class Bilinear(Operator):
    ...
    *see also <Operator> class*
    ...
    def __init__(self,spaceconfig,index1,index2,factor=1.,label=None):
        ...
        self.index1=array(arange(spaceconfig.nflv*2)[index1]).reshape([-1])
        self.index2=array(arange(spaceconfig.nflv*2)[index2]).reshape([-1])
...
class Qlinear_ninj(Qlinear):
    ...
    def __call__(self,param=1.,dense=True,**kwargs):
        '''
